...
object FicusReaders {
  ...
  implicit val paramConfigReader: ValueReader[ParameterConfig] = ValueReader.relative(config => {
    val json = config.root().render(ConfigRenderOptions.concise().setJson(true))
    CirceUtil.decodeJsonUnsafe[ParameterConfig](json, "invalid parameter validator config")
  })
}
